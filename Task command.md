...
> **지금 무엇을 검증해야 하는가**는 [ROLLBACK.md](ROLLBACK.md) §1 에 있다. 이 문서는 상시
> 운영 레퍼런스다.
...
```
🚗 calibrate ─► 🚗 record ─► 🖥 panel_replay / perception_probe (오프라인 실험)
                    │
...
> ⚠ `roi_top_frac` · `trap_top_w` · `trap_bot_w` 는 **삭제됐다.** BEV LUT 자체가 캘리브레이션된
> 사다리꼴 크롭이라 손튜닝 ROI 는 크롭 위의 크롭이었다.
...
ros2 param set /control_node controller        C3     # C1~C5
```

> ⚠ **C4(Pure Pursuit) / C5(Stanley) 는 지금 튜닝할 수 없다.** `lookahead`·`pp_gain`·
> `stanley_k` 등이 ROS 파라미터로 노출돼 있지 않다 (ROLLBACK.md TODO B2). `controller` 만
> 바꿔봐야 기본값으로 돈다.
...
> 입력을 50Hz 로 계속 재발행한다 — 알려진 미수정 구멍 (ROLLBACK.md TODO C1).
...
### C-2. perception_probe — BEV 실험/시각화
```bash
cd offline && ../.venv/bin/python perception_probe.py <raw>.mp4 --stages --name t1
```

### C-3. control_predict / control_select — 제어기 비교
```bash
cd offline
...
../.venv/bin/python control_select.py rslt/pred_<drive>.csv --export C2 \
    --profile ../D-Racer-Kit/src/config/profiles/track2025.yaml
```
> ⚠ **`--camera` 는 필수다.** 예전에는 cam 없이 돌아서 **차가 쓰지 않는 front-view 파이프라인**
> 으로 제어를 예측했다. 그건 모드가 아니라 버그였고, front-view 경로는 삭제됐다.

> ⚠ **open-loop 다.** 다른 조향을 했으면 다른 프레임을 봤을 텐데 그건 재현 못 한다. 후보를
> 걸러내는 데만 쓰고 **최종 판단은 D3-G 폐루프**로 한다 ([CONTROL_DESIGN.md](offline/CONTROL_DESIGN.md) §5.0).
...
#   D3-G: git fetch && git reset --hard origin/kos/track-test2
```
