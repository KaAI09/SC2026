...
**lap 런치 인자 · 회피/미션 켜기:**
```bash
# 검출만 (안 움직임) — 가장 안전. /mission/state·/lane/state 로 확인
ros2 launch dracer_bringup lap.launch.py                        # engage 기본 false

# 미션 스로틀 게이트 (RED/MARK 정지, GREEN 출발) — ⚠ GREEN 볼 때까지 정지로 시작
ros2 launch dracer_bringup lap.launch.py mission_gate:=true

# 갈림길 회피까지 (lap 엔 use_fork launch 인자가 없다 → param 으로)
ros2 param set /perception_node use_fork true      # 안 먹으면 런치 내렸다 다시 올린다
```
> 주요 인자: `engage`(false), `mission_gate`(false), `publish_rate`(30.0), `profile`, `mission_config`.
