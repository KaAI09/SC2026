...
**⚠ `kp 0.75` 는 곡선에서 부족하다** (07142315 측정: 곡선 횡오차 p95 **23cm**, 코리도어 반폭
17.5cm). 조향은 포화되지 않으므로 **올릴 여유가 있다** — §B-7.
...
### B-7. 🚗 제어 튜닝 — `kp` 를 곡선에서 잡는다

**기준선**: [`offline/rslt/07142315`](offline/rslt/07142315) (3세션, engage 1,903프레임).
캘리브·인지·서보 스케일은 **전부 닫혔다.** 남은 것은 `kp` 하나다.

측정 결과 **직선은 완벽하고**(횡오차 0.2cm, 정상상태 조향 −0.004) **곡선에서 벗어난다**
(횡오차 평균 4.7cm, `|ema|` p95 **23cm** — 코리도어 반폭이 17.5cm 다).

**원인은 서보가 아니라 PD 의 구조다.** 일정 곡률의 코너는 **일정한 외란**이고, 적분항 없는
P 제어는 `e_ss = u_필요 / kp` 만큼의 오차를 **반드시** 남긴다.

**그리고 조향에 여유가 있다** — 곡선에서 `|steer|` p90 = 0.65, **포화 0%**. 제어기가 1.0 까지
갈 수 있는데 안 갔다.
...
ros2 param set /control_node throttle_base 0.23    # ⚠ 실차는 0.25 로 달렸다 (프로파일은 0.23)
ros2 param set /control_node kp 1.0
ros2 param set /control_node kd 0.13               # kp 에 비례해 함께 올려 진동 억제
```
