...
**`kp` 1.3 / `kd` 0.145 가 프로파일에서 온다** (실차 교차로 튜닝 최종값 — §B-7).
...
### B-7. 🚗 제어 — 게인 확정 (`kp` 1.3 / `kd` 0.145)

**`kp` 1.3 / `kd` 0.145 는 [`track.yaml`](D-Racer-Kit/src/config/profiles/track.yaml) 에
확정됐다 — 실차 교차로 튜닝 최종값이다.** (07142315 의 `kp` 1.0 상향에서 곡선 추종을 위해 더
올렸다: 그 주행은 직선이 완벽했으나 곡선에서 p95 23cm 로 벗어났고, 조향은 포화되지 않아
`|steer|` p90 0.65 — 올릴 여유가 있었다.) 아래 `param set` 은 **여기서 더 움직여 볼 때** 쓴다.
