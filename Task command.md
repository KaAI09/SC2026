...
> **🔴 0714 현재 D3-G 는 구 코드를 돌리고 있다.** 그 증거: 녹화 csv 의 `LaneState` 컬럼이
> **전부 비어 있고** 파일명이 `raw_<stamp>` 다 — 구 `record.launch` 의 서명이고, 그것은
> **인지를 안 돌린 프레임을 남긴다.** 새 코드는 `collect_`/`drive_` 이고 csv 가 채워진다.
> **아래를 먼저 하지 않으면 이 문서의 절반이 존재하지 않는 런치를 가리킨다.**
...
git fetch origin && git checkout kos/planning
git reset --hard origin/kos/planning
...
> **🔴 신호등 검출은 이 대회장에서 작동하지 않는다. `mission_cls` 의 RED 를 믿지 마라.**
> 0714 실측(4,744프레임): 관중석의 빨간 물체가 **LED 순도로** 찍혀서, 진짜 램프와 **채도·면적·
> 세로위치 세 축이 전부 겹친다.** 신호등이 없는 클립에서 RED 가 **95% 프레임**에 확정된다.
> 어떤 파라미터로도 갈라지지 않는다 (README §7 C1-a).
> **지금은 무해하다** — `MissionGate` 가 `control_node` 에 배선되어 있지 않아 차는 멈추지 않는다.
> **방향표지판은 정확하다** (LEFT 0.92 / RIGHT 0.93).
>
> 해결하려면 **실제 설치된 신호등을, 실제 정지선 거리에서** 녹화해야 한다 — 0714 데이터의 램프는
> 사람이 손에 들고 팔 길이에서 보여준 것이라 차가 마주할 기하가 아니다.
> (`mission_config:=<venue.yaml>` 인자는 살아 있다. 다만 그 YAML 을 써 줄
> `scripts/mission_tune.py` 는 **아직 없다.**)
...
### B-7. 🚗 트랙 검증 (**미완. 기준선이 없다 — 이걸 통과해야 생긴다**)

**옛 기준선(0712)은 폐기됐다.** 그 주행은 차선폭을 16~21cm 로 재던 `camera.yaml` 위에서
이뤄졌다 — 새 캘리브로는 같은 코드가 직선 구간에서 쌍검출 **36.4% → 100%** 를 낸다.
그 주행의 수치는 지금 코드의 성능을 말해주지 않는다.

**`kp` 가 두 가지 이유로 미검증이다:**
1. **A1** — 서보 실측으로 조향 스케일이 `41.67u` → `25u` 가 됐다 (`kp` 0.45 → **0.75**,
   `slew` 4.5 → **7.5**, `steer_max` 0.7 → **1.0**). **계산상 등가일 뿐이다.**
2. **새 캘리브** — `y_near` 가 26 → **34cm** 다 (카메라가 더 위를 본다). `center_error` 를
   재는 지점이 8cm 멀어졌다 = **preview 가 늘었다.** 같은 `kp` 가 같은 뜻이 아니다.

인지(B2·B3)는 오프라인 등가 검증을 통과했다 — 트랙에서 한 번 돌려 확인만 하면 닫힌다.

```bash
# A-2 를 먼저 하라. --packages-select 금지 (MissionState.msg 신설).
...
**② 인지 (B2 · B3 + 새 캘리브) — 액추에이션 없음**
```bash
ros2 launch dracer_bringup collect.launch.py
ros2 topic hz /lane/state          # 30Hz (collect 는 렌더링을 안 한다 = 주행과 같은 레이트)
ros2 topic echo /lane/state --once # state=OK, valid=true
```
**오프라인 재생(0714, 12세션 4,744프레임)이 예측하는 값** — 실차가 이것과 크게 다르면 멈춰라:

| | 기대 |
|---|---|
| `valid` | 95 ~ 100% |
| `state` | OK 93~100%, `LOST` 0 |
| 쌍검출 | 직선 100% · 급커브 8~16% (**정상이다** — 곡선 바깥 경계가 FOV 에 없다 → `coast`) |
...
```
> ⚠ **색 임계는 건드리지 마라.** 0714 주행 데이터로 측정했고 **현재 값이 실측 분포를 완전히
> 감싼다** (README §7 "하지 마라"). `color_gate`·`pair_same_color`·`lane_width_cm`·`x_half` 도
> A/B 로 **전부 효과 0** 이었다.

**③ 저속 자율 주행 — ⚠ 여기가 진짜 시험이고, 이 문서에서 유일하게 미지수인 곳이다**
```bash
ros2 launch dracer_bringup drive.launch.py
ros2 topic hz /lane/state                          # 30Hz 유지되는가
# 바퀴 띄운 상태로 engage(A) 확인 -> 내리고 저속
```
`kp 0.75` 는 **A1(스케일 1.667배)과 새 캘리브(preview +8cm) 두 가지 이유로 미검증**이다.
저속·감시 하에서:
...
**④ 성공 시 — 기준선을 만든다** (지금은 **없다**)
```bash
# START 로 녹화하고 raw+csv 를 로컬로 (§E)
# panel_replay --csv 로 재구성 -> README §6 에 새 기준선을 적는다
```
> 이 주행이 **처음으로** 새 캘리브 + 새 조향 스케일에서 나온 수치가 된다.
> csv 의 `state` 컬럼이 **채워져 있는지** 확인하라 — 비어 있으면 구 코드다 (§A-2).
