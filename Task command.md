...
실차 트랙 테스트의 전 과정. 작업 브랜치 **`kos/out-course`**. 실행 위치를 구분한다.
...
> 운영 레퍼런스다. (구 `lap_command.md` 는 이 문서 §B-4 로 통합됐다.)

> **현재 설정 요약** (프로파일 [`track.yaml`](D-Racer-Kit/src/config/profiles/track.yaml) · [`camera.yaml`](D-Racer-Kit/src/config/camera.yaml) 실측):
>
> | 항목 | 값 | 비고 |
> |---|---|---|
> | camera.yaml `x_half_cm` | **29cm** | 넓은 42cm 는 center_error 를 0.69배 축소 → 조향 약화. 쓰지 않는다 |
> | `kp` / `kd` | **1.5 / 0.145** | 07160644 실차 스윕 채택 (곡선 e_ss 8cm). ⚠ 급코너 조향포화 병목이라 더 안 올림 |
> | `throttle_base` / `min` | **0.23 / 0.22** | |
> | `colors` | **[white]** | ⚠ 노랑 OFF(임시) — 노란 인코스 오판 방지. 되돌리려면 [white, yellow] |
> | `use_fork` | **true (기본)** | 갈림길(W-W 섬) 감지 시 표지판 hint 대로 corridor 선택 |
> | `sign_invert` | **false (기본)** | 이 대회장은 원본 정확(좌→4/우→3). 뒤집히는 대회장서만 `param set` |
> | `mission_gate` | **on (기본)** | RED/MARK 정지·GREEN 출발. ⚠ ON=GREEN 볼 때까지 안 움직임(정지 시작). 끄려면 `mission_gate:=false` |
...
> `camera.yaml`)를 커밋값으로 덮어쓰고 녹화 데이터(`recorder/`, `offline/rslt/`)도 지운다.
> **백업 먼저.**
...
grep -E "SERVO_|ACCEL_RATIO" D-Racer-Kit/src/config/vehicle_config.yaml   # 값도 눈으로 적어둬라

# 2) 원격(kos/out-course)과 완전 동기화 + 빌드 삭제
git fetch origin
...
# 3) 보드 고유 캘리브 복원 (실차 정확성 필수 — 완전 초기화를 원하면 생략)
...
# 4) 재빌드 (⚠ 전체 빌드. dracer_msgs 의 LaneState/MissionState 가 바뀌었다)
cd D-Racer-Kit
source /opt/ros/humble/setup.bash
colcon build --symlink-install
source install/setup.bash
```
> ⚠ **`--packages-select` 로 부분 빌드하지 마라.** `dracer_msgs` 의 `LaneState.msg`(sign_hint/
> sign_hint_age 추가)·`MissionState.msg` 를 모르는 채 빌드된 노드는 **조용히 옛 인터페이스로** 돈다.
> 부분 빌드가 꼭 필요하면 `dracer_msgs` 를 **먼저** 빌드하고 소싱한 뒤 나머지를 빌드한다.
>
> 새 터미널마다: `cd ~/SC2026/D-Racer-Kit && source /opt/ros/humble/setup.bash && source install/setup.bash`
...
```
frame_time,
valid, center_error, center_error_cm, ema,   ← center_error_cm = calib 불변 cm 횡오차
heading_valid, heading, confidence,
left_conf, right_conf, state, used_fallback,
n_corridors, ego_rule,                       ← 분기 증거 (ego_rule 에 fork_L/fork_R 포함)
fork_type, n_islands,                        ← 갈림길 감지 (island/branch/'')
sign_hint, sign_hint_age,                    ← 표지판 방향 힌트('' /L/R)+나이. age>sign_live_hold 면 만료
...
```
> **sign→branch 진단 체인** (우회전 '무조건 좌회전' 규명용): `mission_det_cls`(프레임별 raw
> 화살표 3/4) → `mission_cls`(확정) → `sign_hint`+age(래치·투입된 방향) → `fork_type`/`n_islands`
> (섬 감지 = 축B) → `ego_rule`(fork_L/R 이면 hint 가 corridor 선택에 쓰임) → `ctrl_steering`(결과).
...
```bash
ros2 launch dracer_bringup drive.launch.py                    # engage=false + mission_gate=true(기본)
ros2 launch dracer_bringup drive.launch.py mission_gate:=false  # 게이트 끄고 순수 주행 확인할 때
```
> ⚠ **mission_gate 가 기본 ON 이다.** engage 해도 **GREEN 을 볼 때까지 안 움직인다**(정지로 시작).
> 신호등 없이 주행만 확인하려면 `mission_gate:=false`. "안 움직임"은 고장이 아니라 GREEN 대기다.
...
**`kp` 1.5 / `kd` 0.145 가 프로파일에서 온다** (07160644 실차 스윕 채택 — §B-7).
...
**lap 런치 인자 · 회피/미션:**
```bash
# 기본 = 미션 게이트 ON (RED/MARK 정지·GREEN 출발). ⚠ GREEN 볼 때까지 정지로 시작
ros2 launch dracer_bringup lap.launch.py                        # engage 기본 false

# 미션 게이트 끄기 (검출만, throttle 안 막음) — 게이트 없이 순수 주행 확인할 때
ros2 launch dracer_bringup lap.launch.py mission_gate:=false

# 갈림길 회피는 use_fork(프로파일 기본 true). 런치 인자 없으니 필요 시 param 으로
ros2 param set /perception_node use_fork true      # 안 먹으면 런치 내렸다 다시 올린다
```
> 주요 인자: `engage`(false), **`mission_gate`(true)**, `publish_rate`(30.0), `profile`, `mission_config`.
> lap 은 `debug_view` 를 강제 off(최경량) — 확인은 **토픽 echo** 로 한다.

**토픽 확인 (별도 터미널):**
```bash
ros2 topic echo /mission/state --field cls    # -1 없음·0 GREEN·1 RED·2 MARK·3 RIGHT·4 LEFT
ros2 topic echo /lane/state --field fork_type # ''|island|branch — 섬 감지
ros2 topic echo /lane/state --field ego_rule  # fork_L/fork_R 이면 회피 조향 중
ros2 topic echo /lane/state --field sign_hint # ''|L|R — 래치된 표지판 방향
ros2 topic echo /control                      # 실제 기동(steering/throttle)
```

| 확인할 것 | 토픽·필드 | 기대 |
|---|---|---|
| 섬 검출 | `/lane/state` `fork_type` | 섬 구간 `island` |
| 방향 래치 | `/lane/state` `sign_hint` | 표지판 본 뒤 `L`/`R` |
| 회피 기동 | `/lane/state` `ego_rule` | 표지판+섬이면 `fork_L`/`fork_R` |
| 미션 검출 | `/mission/state` `cls` | 3/4 표지판·0/1 신호등·2 ArUco |

> 회피는 **표지판을 본 뒤 sign_live_hold(12프레임 ≈ 0.4초) 안에 섬이 나타날 때만** 작동한다.
> cls 가 3/4 로 뜬 직후 `sign_hint` 와 `ego_rule` 을 같이 본다.

**진단 로그를 txt 로 저장** (`tee` = 화면+파일 즉시 기록, Ctrl+C 종료):
```bash
# 미션 전체(타임스탬프+cls+det_cls) — 표지판 검출 분석
...
> (⚠ 현재 우회전 '무조건 좌회전' 진단 중 — det_cls 가 우회전에서도 4 로 나오면 축A 화살표 오독.)
...
ros2 topic echo /lane/state --field ego_rule        # tracked / nearest / coast / fork_L / fork_R / branch_*
```
```bash
...
### B-7. 🚗 제어 — 게인 확정 (`kp` 1.5 / `kd` 0.145)

**`kp` 1.5 / `kd` 0.145 는 [`track.yaml`](D-Racer-Kit/src/config/profiles/track.yaml) 에
확정됐다 — 07160644 실차 스윕 채택.** 풀랩(211938/212037, kp 1.5)에서 곡선 e_ss **8.0~8.6cm**
로 코리도어 반폭(17.5cm) 안, 진동 0.7~1.3/s, 차선이탈 0. ⚠ **급코너 조향포화 12~26% 가 병목**
이라 kp 를 더 올리면 안 된다 — 다음 레버는 `curv_slow`(급코너 감속). 아래 `param set` 은
**여기서 더 움직여 볼 때** 쓴다.

```bash
ros2 launch dracer_bringup drive.launch.py         # engage=false 로 시작. kp/kd 는 프로파일에서 온다
# 바퀴 띄운 상태로 engage(A) 확인 -> 내리고 저속 -> START 로 녹화

# 라이브 튜닝 (Tracker/EMA 상태 보존됨):
ros2 param set /control_node kp 1.5                # 곡선에서 여전히 벗어나면 (|steer| 1.0 닿으면 한계)
ros2 param set /control_node kp 1.1                # 진동하면 낮춘다
ros2 param set /control_node throttle_base 0.23    # 여러 throttle 로 달려 ctrl_feedback 추세 확보
```

> **판정 기준**: 곡선 `|ema|` p95 **< 17.5cm**(코리도어 안) · 조향 포화 없음(`|steer|` 1.0 미접촉)
> · 진동 안 늘어남 · 직선 그대로(횡오차 ~0.2cm). START 로 녹화 → `panel_replay --csv`(§C-1) +
> `ctrl_feedback`(§C 오프라인)로 확인. **여러 throttle(0.19/0.23/0.26) 로 ≥3세션** 쌓으면
> ctrl_feedback 이 throttle↔포화 추세를 피팅한다.
...
| `center_error_cm` | **cm 단위 횡오차 (calib 불변)** — metric 제어·분석은 이걸 쓴다 |
...
| **`ego_rule`** | **무엇이 골랐나: `tracked` / `nearest` / `coast` / `fork_L` / `fork_R` / `branch_random` / `none`** (fork_L/R = use_fork 회피 조향) |
| `fork_type` / `n_islands` | 갈림길 감지: `''`/`island`/`branch` · 이 프레임 island 코리도어 수 |
| `sign_hint` / `sign_hint_age` | 래치된 표지판 방향 `''`/`L`/`R` · 확정 후 프레임 수 (age > `sign_live_hold` 면 만료·미투입) |
...
#   D3-G: git fetch && git checkout -f -B kos/out-course origin/kos/out-course  (§A-2 캘리브 백업 먼저)
```
