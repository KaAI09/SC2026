...
```text
Camera Node ─► Perception Node ─┬─ /lane/state ────► Control Node ─► /control ─► Actuator Node ─► Motor/Steering
                                └─ /mission/state ──►                                              (E-STOP 여기서)
Joystick Node ─► /joystick ─────────────────────────► Control Node
Camera / Battery ───────────────────────────────────► Monitor Node ─► Web Dashboard (:5000)
```
...
이 저장소의 현재 자율주행 시스템은 카메라 한 장을 **캘리브레이션된 metric BEV**로 펴서 차선을
검출하고, 자차 통로(코리도어) 중앙선을 추정해 조향·스로틀로 매핑한다. 신호등·ArUco·방향표지판
인식과 갈림길(fork) 회피를 포함한다. 시스템 구조·인지·제어·현재 상태·남은 작업은 저장소 루트
[README.md](../README.md), 운영 명령은 [Task command.md](../Task%20command.md) 참고.

> 아래는 D-Racer Kit 의 원본 OpenCV(Canny) 차선추종 데모(WaveShare PiRacer 트랙)로, 현재
> metric-BEV 파이프라인의 전신이다.
...
| Package | Description |
|---|---|
| `camera` | 카메라 이미지 publish → `/camera/image/compressed` |
| `perception` | **차선 + 객체 인지** (같은 프레임·같은 stamp) → `/lane/state` + `/mission/state` |
| `control` | 제어 로직 (LaneState → `/control`) + engage/E-stop 안전 게이팅 |
| `actuator` | throttle / steering 서보 구동. **E-STOP 이 여기서 걸린다** |
| `joystick` | 조이스틱 입력 처리 및 수동주행 + 캘리브레이션 |
| `recorder` | 동기 주행 영상(mp4) + 로그(csv) 기록 (**rosbag 아님**) |
| `monitor` | 웹 기반 상태 모니터 + 라이브뷰 (:5000) |
| `battery` | 배터리 상태 publish |
| `dracer_core` | 순수(ROS 무관) 인지·제어·객체검출 코어 (`perception_core`·`control_core`·`mission_core`) |
| `dracer_msgs` | custom messages (LaneState·MissionState·Control·ControlConfig·Joystick·Battery) |
| `dracer_bringup` | 런치 파일 (calibrate·collect·drive·lap) |
| `config` | 차량·카메라·프로파일 설정 (`vehicle_config.yaml`·`camera.yaml`·`profiles/`) |
