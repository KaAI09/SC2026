...
ros2 launch control online_auto.launch.py \
    profile:=$PWD/src/config/profiles/track2025.yaml   # engage=false로 시작
```
이 런치는 `perception_node`→`driving_node`→`control_node`(`use_joystick_control=False`)
경로로 동작하며, 구동은 `engage` 게이트로 막혀 있습니다.
(이때, 조이스틱은 E-STOP 용도로 동작합니다. 바퀴를 띄운 상태로 방향 확인 후
`ros2 param set /driving_node engage true`로 구동을 시작하세요.)
### 3-4. 노드 단독 실행 예시
...
- 트랙 테스트 파이프라인: `calibrate` → `record_manual` → `online_manual` → `online_auto` (Track test pipeline 참고)
- 자율 주행 테스트: `online_auto.launch.py` (perception→driving→control, `engage` 게이트로 시작)
