...
| [offline/README.md](offline/README.md) | 오프라인 도구 (panel_replay · bev_eval · ctrl_feedback · calibrate) |
...
**판단 계층은 부분적으로 존재한다.** `use_fork`(기본 true)가 켜지면 **갈림길(W-W 섬)에서
표지판 `sign_hint` 방향으로 한쪽 corridor 를 고른다**(`ego_rule` = `fork_L`/`fork_R`). 그 외의
일반 분기에서는 아직 `tracked`(이어가기)/`nearest`(자리표시자)에 머문다 — 완전한 route 판단은
남은 작업 C2. `sign_hint`/`fork_type` 은 **우회전 표지판 진단**(현재 작업, §7 C)을 위해 로그로
노출된다.
...
| 제어기 | 식 | 상태 |
|---|---|---|
| **PD** | `−(kp·e + kd·ė)` | ✅ **기본** (`kp 1.5`, `kd 0.145` — 07160644 실차 스윕 채택, 곡선 e_ss 8cm). 0712 `kp 0.45` → A1 서보 스케일 1.667배 → 곡선 추종 위해 상향 |
...
> `slew_rate_per_sec` 4.5 → **7.5**.
> **실차로 확인됐다** (07142315): 조향 포화 **0%**, 직선 정상상태 조향 **−0.004** ≈ 0 —
> `u = 0` 이 곧 직진이고 `steer_max` 1.0 이 전 구간 선형이다.
> **다만 `kp` 0.75 는 곡선에서 부족하다** — §7 A1.
>
...
- `bev_eval.py` — BEV/calib 품질 오프라인 평가 (`compare`/`quality`/`coast`)
...
- `fork_probe.py` — 갈림길(island) 감지율 + 일반주행 오검출률 검증
...
### 🚗 자율주행 기준선 — `offline/rslt/07142315` (3세션, **`kp` 0.75 시점**)

> ⚠ **이 기준선은 옛 게인(`kp` 0.75)이다.** 현재 프로파일은 `kp` **1.3** / `kd` **0.145**
> (실차 교차로 튜닝 최종, §7 A). 아래 곡선 수치는 그래서 **개선 전** 값이다 — fresh 주행은
> `offline/rslt/recorder(07160355)/`.

**첫 실차 자율주행.** 아래는 engage 구간 1,903프레임의 실측이다.

| | |
|---|---|
| 인지 레이트 | **30~31 Hz** |
| `state` (engage 중) | **OK** — `LOST` · `OUTLIER` **0회** |
| 실차 ↔ 오프라인 재생 | ✅ **재현 일치** (`\|Δcenter_error\|` 중앙값 0.012) |
| 조향 포화 | **0%** (`\|steer\|` p90 = 0.65, max 0.95 / `steer_max` 1.0) |
| **직선** 횡오차 | **+0.2cm** (정상상태 조향 −0.004 ≈ 0) |
| **곡선** 횡오차 | **+4.7cm** 평균 · **p95 23cm** ← 문제는 여기다 |

**직선은 완벽하고, 곡선에서 벗어난다.** 코리도어 반폭이 17.5cm 인데 `|ema|` p95 가 23cm 다 —
차가 실제로 코너에서 트랙을 벗어난 구간이 있다.

> **이것은 서보 트림이 아니다.** 직선 정상상태 조향이 −0.004 로 사실상 0 이다 (A1 이 트림을
> 서보 중립으로 옮긴 것이 실차에서 확인됐다). 원인은 **PD 레귤레이터의 구조**다: 일정 곡률의
> 코너는 **일정한 외란**이고, 적분항 없는 P 제어는 `e_ss = u_필요 / kp` 만큼의 오차를
> **반드시** 남긴다. 곡선 `ema` 0.16 이 정확히 그 값이다.

### 인지 — 검증 완료

| | 최종 하드웨어 (직선 708f) | 대회장 수동주행 (12세션 4,744f) |
|---|---|---|
| `valid` | **100%** | 95 ~ 100% |
| 쌍검출 / coast | **96 ~ 100%** / 0~4% | 8 ~ 100% / 0~94% |
| `state` | **OK 100%** | OK 93 ~ 100%, `LOST` 0 |

> **쌍검출률이 구간마다 크게 다르다.** 직선은 100%, 급커브는 8% 다. 낮은 구간에서 일어나는
> 일은 실패가 아니라 **곡선 바깥 경계가 FOV 에 없는 것**이고, 그때 `coast` 가 정확히 그 일을
> 한다 — 그 클립들도 `state` 는 OK 다. 색·게이트·폭·BEV폭 파라미터는 **A/B 로 전부 기각됐다**
> (§7 "하지 마라").

> ⚠ **노랑 `H` 하한의 마진이 사라졌다.** 최종 하드웨어에서 노란 테이프의 `H` 가 **18~27** 로
> 나온다 (`yellow_h_lo` 가 정확히 **18**). 옛 카메라에서는 19~32 였다. **지금은 통과하지만**
> 조명이 어두워지면 걸릴 자리다. 걸리면 `yellow_h_lo` 를 14 로 내려라 (라이브 `param set`).

### 캘리브레이션 (`camera.yaml`, 07142150) ✅ 검증 완료

**렌즈(K·D)까지 다시 풀었다** (체커보드 19장, 재투영 RMS **0.533px**).
...
| **`--check`** | **직선 5프레임 전부 `OK`** — 폭 오차 −0.8~+0.6cm · 평행성 0.8~1.2cm · 수직성 1.0~2.5cm |

**`near` 가 줄자와 맞는다** — H 가 옳다는 가장 직접적인 증거다:
...
**채택: `ground_03`.** 고르는 기준은 차선폭의 **평행성**이다. 보드가 40cm 이상 멀어진 세 장은
차선폭이 35 → 20cm 로 **붕괴한다** — 근거리 `H` 가 무너지는 것이고, 그 세 장도 **혼자 보면
"카메라 높이 정합, RMS 양호"** 로 통과한다. 5장을 서로 대조하는 것만이 그것을 잡는다.

> **차선폭 34.8cm 는 두 번 독립적으로 나왔다.** 0714 캘리브(옛 하드웨어, 옛 K·D)에서 34.78cm,
> 07142150(새 하드웨어, 새 K·D)에서 34.76cm. 서로 다른 렌즈 파라미터에서 같은 값이 나온다는
> 것은 **이 트랙의 실제 차선폭이 약 34.8cm** 라는 뜻이다.

---

## 7. 남은 작업

### 🟢 A. 제어 — 게인 확정 (`kp` 1.5 / `kd` 0.145)

- [x] **A1. `kp` 0.45 → 0.75(A1 서보 스케일) → 1.0 → 1.3 → 1.5 · `kd` → 0.145.**
      [`track.yaml`](D-Racer-Kit/src/config/profiles/track.yaml) 에 확정. 최신 근거
      (07160644 실차 스윕, `kp` 1.5 풀랩): 곡선 e_ss **8.0~8.6cm** 로 코리도어 반폭(17.5cm) 안,
      진동 0.7~1.3/s, 차선이탈 0. PD 는 일정 곡률 코너에서 `e_ss = u_필요/kp` 를 **반드시**
      남긴다. ⚠ **급코너 조향포화 12~26% — 병목이라 kp 를 더 올리면 안 된다** (curv_slow 가 레버).
- [ ] **A2. 오프라인 게인 되먹임 도구 완성** — `offline/ctrl_feedback.py` 가 주행 로그에서
      곡선 e_ss·진동·조향포화를 진단하고 권장 kp 를 낸다. **여러 throttle(0.19/0.23/0.26)로 ≥3세션**
      쌓으면 throttle↔포화 추세를 피팅한다 (실차 fresh 주행 필요).

### 🟠 B. 기능

- [ ] **B1. 제어기 재설계 — metric Pure Pursuit + 곡률 감속 + anti-windup.** *(우선순위 낮음)*
      A1 의 `kp` 상향은 **증상 완화**다. `center_error` 를 **전방 31cm 한 점**에서 재는 순수
      횡오차 레귤레이터인 한 코너 진입 반응은 구조적으로 늦고, 속도를 올리면 진동한다. PP 는
      **78cm 앞까지의 중앙선 다항식**(이미 손에 있다)에서 전방주시 횡오차를 읽어 곡률을 직접
      계산하므로, 게인이 아니라 **측정값**(휠베이스·최대 조향각)으로 돈다. `κ` 가 공짜로 나오니
      **곡률 기반 감속**도 함께 붙는다 (지금 `curv_slow` 는 곡률이 아니라 `|steering|` 에
      곱해져 **결과에 반응**한다 — 위상이 늦다). PID anti-windup 도 여기서 함께 정리한다.
      **전제는 전부 채워졌다**: 최대 조향각 **25도**(A1 실측) · 휠베이스 **17.5cm**(실측).
      남은 것은 `LaneState` 에 lookahead 횡오차 필드를 추가하는 **인지↔제어 계약 변경**뿐이다.
      > ⚠ **전방(y) BEV 스케일에 ~5% 오차가 있다.** 횡(x)은 1.7% 인데 전방만 그렇다. 지금
      > 제어는 횡오차만 읽으니 무해하지만 **PP 는 `Ld` 를 실제 cm 로 쓴다 → 5% 거리 오차 =
      > 5% 곡률 오차다.** PP 착수 전에 보드를 지면에 **직접 눕혀서**(부양 0cm) 다시 찍어라.

### 🟡 C. 미션

- [x] **C1. 갈림길 회피 (`use_fork`).** W-W 섬을 `(L,R)` 반대곡률 + spread/gap 게이트로 감지하고,
      표지판 `sign_hint` 방향으로 섬 바깥 한쪽 corridor 를 고른다(`ego_rule` = `fork_L`/`fork_R`).
      일반구간 오검출 0% 실측. 기본 true.
- [x] **C3. 우회전 표지판 좌/우 구분 — 해결 확인 (07160644).** 옛 증상(좌/우 무관 무조건
      좌회전)은 fork 개선(`dacc6a6`)+`use_fork` true 로 해소됐다. 07160644 실차 데이터에서
      RIGHT 세션(det_cls 3)은 전부 `ego_rule=fork_R`+우조향(mean −0.41), LEFT 세션(det_cls 4)은
      전부 `fork_L`+좌조향(mean +0.20), **교차 오류 0**. 검출→확정→회피규칙→물리 조향까지 좌/우
      정확. (sign_hint 계측은 이후 우회전 미검출 등 다른 실패 모드가 나올 때를 위한 로그로 유지.)
- [ ] **C2. 판단 계층** — **분기에서 `tracked` 규칙을 오버라이드하는 것.** 측정 완료: 분기
      **218프레임(3.0%), 28회, p50 6프레임, max 20 (0.7초)**. 현재 분기에서 `tracked` 가 98% 를
      ...
      > `MissionGate` 는 아직 `control_node` 에 배선되어 있지 않다 (검출·발행만 한다).

### ⚪ 하지 마라 — 측정으로 기각됨

- **색 임계 튜닝** — **이미 맞다.** 차가 실제로 보는 것(320x240 주행 클립)의 BEV 내 분포:
  노랑 H **18~27** (임계 18~36 ✅) · S p05 85 (≥65 ✅) · V p05 133 (≥100 ✅),
  흰 S p98 26 (≤60 ✅) · V p02 207 (≥185 ✅).
