...
| 제어기 | 식 | 상태 |
|---|---|---|
| **PD** | `−(kp·e + kd·ė)` | ✅ **기본** (`kp 1.3`, `kd 0.145` — 실차 교차로 튜닝 최종값). 0712 `kp 0.45` → A1 서보 실측으로 스케일 1.667배 → 곡선 추종 위해 상향 |
...
### 🟢 A. 제어 — 게인 확정 (`kp` 1.3 / `kd` 0.145)

- [x] **A1. `kp` 0.45 → 0.75(A1 서보 스케일) → 1.0 → 1.3 · `kd` → 0.145.**
      [`track.yaml`](D-Racer-Kit/src/config/profiles/track.yaml) 에 확정, 실차 교차로 튜닝
      최종값. 근거 (07142315, `kp` 0.75): 직선 완벽(횡오차 0.2cm)하나 곡선에서 정상상태 오차를
      남기고 `|ema|` p95 23cm (코리도어 반폭 17.5cm). PD 는 일정 곡률 코너에서 `e_ss = u_필요/kp`
      를 **반드시** 남기는데 조향은 포화 없어(`|steer|` p90 0.65) 올릴 여유가 있었다 → 상향.
...
- [ ] **C3. 우회전 표지판 진단 (현재 작업).** 증상: **좌/우 표지판과 무관하게 교차로에서 무조건
      좌회전.** 원인은 축A(`_arrow_direction` 이 우회전 화살표를 오독) 또는 축B(교차로에서 섬
      미감지 → nearest 가 좌측)로 좁혀졌다. **진단 계측 완료** — `LaneState.sign_hint`/`sign_hint_age`
      + recorder CSV(fork_type/n_islands/sign_hint)로 pixel→화살표→hint→fork→corridor→조향 체인을
      로그에서 추적한다. **우회전 표지판 fresh 클립**으로 축A/B 확정 후 fix 설계.
