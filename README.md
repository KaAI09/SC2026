...
프로파일을 흔들어 볼 때 되풀이하지 않도록 남긴다 — 아래는 전부 A/B 로 효과 0 이거나 역효과였다.

- **색 임계 튜닝** — 차가 보는 320x240 스트림의 BEV 분포로 재면 이미 임계 안이다. 고해상도 지면
  사진(960x720)은 노랑 H 를 다르게 재서 거짓 결론을 준다. **차가 보는 것으로 재라.**
- **`color_gate` 끄기·낮추기** — 죽는 색은 이미 코리도어를 못 만들던 색이다. 꺼도 쌍검출률 불변.
- **흰-노랑 쌍 허용** — 6개 클립 전부 쌍검출률이 소수점까지 동일. 효과 0.
- **쌍검출률을 소프트웨어로 올리기** — 급커브 coast 는 진짜 FOV 한계다. 없는 차선은 어떤
  파라미터로도 만들 수 없고, 그 클립들의 `state` 는 이미 OK 다.
- **연속 confidence** — 틀린 coast 는 기하학적으로 완벽해서 quality 와 오차의 상관이 반대다.
- **핫패스 최적화** — `cv2.inRange`·2채널 remap 모두 오히려 느려진다. 파이프라인은 0.72ms /
  33ms 예산이다.
- **평행도를 페어링 게이트로** — 노란 갈림길을 지운다(§3.2).
