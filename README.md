...
- [x] **A2. `camera.yaml` — 0714 대회장 지면 사진으로 재산출. 🚗 `--check` 대기.**
      ...
      **채택: `ground03.png`** (`--board-offset-cm 1.5` = 보드가 지면에서 뜬 높이).
      ...
      > ⚠ **`lateral_offset_cm` 이 미확정이다.** 5장 전부 보드 횡위치가 **+3.3~+4.4cm** 로
      > 일관되게 나온다. 보드를 오른쪽에 놓은 것이면 무시해도 되지만, **차량축에 맞춰 놓은
      > 것이라면 카메라 광축이 차량축에서 4cm 틀어져 있다는 뜻**이고, 그러면 차가 차선 중앙
      > 에서 **4cm 오른쪽으로 치우쳐 달린다.** 지금은 `0.0` 이다. `--check` 의 **수직성**
      > (차량축이 차선 중앙에 오는가)이 판정한다.
