...
> ⚠ **이 기준선은 옛 게인이다.** 현재 프로파일은 `kp` **1.0** / `kd` **0.13** 이고 (§7 A1),
> 그 게인으로 달린 주행은 **녹화되지 않았다.** 아래 곡선 수치는 그래서 **개선 전** 값이다.
...
### 🟢 A. 제어 — `kp` 1.0 적용 완료 (계측만 남았다)

- [x] **A1. `kp` 0.75 → 1.0 · `kd` 0.1 → 0.13.** [`track.yaml`](D-Racer-Kit/src/config/profiles/track.yaml)
      에 반영했고 **실차 주행으로 확인했다.**
      근거 (07142315, `kp` 0.75): 직선은 완벽했으나(횡오차 0.2cm) 곡선에서 12.8cm 의 정상상태
      오차를 남기고 `|ema|` p95 가 23cm 까지 갔다 (코리도어 반폭 17.5cm). PD 는 일정 곡률의
      코너에서 `e_ss = u_필요/kp` 를 **반드시** 남기는데, **조향은 포화되지 않았다**
      (`|steer|` p90 0.65, 포화 0%) — 올릴 여유가 있었다.
...
- [ ] **A2. 그 주행의 수치가 없다 — 다음 주행에서 녹화하라.**
      `kp` 1.0 주행은 **녹화되지 않았다.** 개선은 눈으로 봤지만 **측정된 적이 없다** — 이
      레포에서 "관찰"과 "검증"은 다른 것이고, A1 이 그것을 이미 한 번 보여줬다("계산상 등가"
      라고 적어둔 값이 실차에서 "곡선에서 부족하다" 로 판명됐다).
      **START 로 한 세션만 녹화하면 닫힌다.** 판정 기준은 이미 정해져 있다:

      | 확인 | 기대 |
      |---|---|
      | 곡선 `\|ema\|` p95 | **< 17.5cm** (= 코리도어 안에 머문다. 옛 값 23cm) |
      | 조향 포화 | `\|steer\|` 가 1.0 에 닿지 않는다 (닿으면 `kp` 과다) |
      | 진동 | 조향 부호반전이 늘지 않는다 (옛 값 1.0~1.2회/초) |

      ```bash
      # panel_replay --csv 로 확인 (Task command §C-1)
      ```
