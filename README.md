...
### 🚗 자율주행 기준선 — `offline/rslt/07142315` (3세션, 새 캘리브 + A1 조향 스케일)
...
### 🔴 A. 제어 — `kp` 가 곡선에서 부족하다 (유일한 실차 이슈)

- [ ] **A1. `kp` 0.75 → 1.0 재튜닝.** 07142315 주행이 답을 줬고, **그 답은 "부족하다"** 다.
      곡선에서 12.8cm 의 정상상태 오차를 남기고 `|ema|` p95 가 23cm 까지 간다 (코리도어 반폭
      17.5cm). **그런데 조향은 포화되지 않았다** — `|steer|` p90 0.65, 포화 0%. 제어기가 1.0
      까지 갈 수 있는데 안 갔다. **올릴 여유가 있다.**
      ...
      ```
      **⚠ 실차는 `throttle_base` 0.25 로 달렸다.** 프로파일 값은 0.23 이다. 코너 이탈에는
      속도도 기여하므로 0.23 으로 되돌리고 `kp` 를 시험하라.
      진동(조향 부호반전)이 현재 1.0~1.2회/초다 — `kp` 를 올리면 늘어난다. 저속·감시 하에서.
