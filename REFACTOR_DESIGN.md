...
- 확정 파이프라인만 유지: `calibrate` · `record_manual` · `online_manual` · `online_auto`.
...
| `record_manual` | camera·actuator·joystick·recorder(raw) | 오프라인용 원본 영상 |
| `online_manual` | camera·perception·actuator(joystick)·joystick·recorder·(tuner) | 인지 검증 + 데이터 수집 + live 튜닝 |
| `online_auto` | camera·perception·control·actuator·joystick·recorder·(tuner) | 자율주행(engage A) + live 튜닝 |
...
| 11 | `dracer_bringup` | py | 런치 4개 (calibrate·record_manual·online_manual·online_auto) |
...
│  │  │  ├─ launch/{calibrate,record_manual,online_manual,online_auto}.launch.py
│  │  │  └─ setup.py · package.xml
│  │  ├─ config/                      (유지)
│  │  │  ├─ vehicle_config.yaml
...
├─ Track test command.md · Track test pipeline.md
