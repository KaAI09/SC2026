...
| `bev_eval.py` | BEV/calib 품질 오프라인 평가 (`compare`/`quality`/`coast` 3서브커맨드) |
| `ctrl_feedback.py` | 주행 로그 → 제어 진단·권장 kp·병목 식별 + throttle 추세 누적 |
| `calibrate.py` | 체커보드·지면 사진 → `camera.yaml` (K/D + 호모그래피 H) |
| `lane_color_probe.py` | 대회장 조명에서 흰/노랑이 **HSV 어디 있는지 측정** → 임계값 제안 |
| `color_gate_probe.py` | `color_gate` 가 진짜 차선을 지우는지 센다 |
| `fork_probe.py` | 갈림길(island) 감지율 + 일반주행 오검출률(목표 <1%) 검증 |
...
| 경로 | 내용 |
|---|---|
| `rslt/recorder(07160355)/` | **실차 fresh 기준선** (raw mp4 + csv + `.meta.json` 사이드카 + `panel/` 디버그영상) |
| `calib/` | 캘리브레이션 사진 |
...
## bev_eval — BEV/calib 품질을 재는 도구

`panel_replay` 와 같은 재생 하네스를 공유하되, **calib 자체의 품질**을 판정한다. 서브커맨드 셋:

```bash
# compare — 두 camera.yaml 로 같은 클립 재생. center_error 는 x_half 비만큼 달라지고
#           (정규화가 calib 에 결합) center_error_cm 은 불변인지(디커플링) 확인
.venv/bin/python offline/bev_eval.py compare <raw.mp4> \
    --camera-a <current>.yaml --camera-b <fresh>.yaml \
    --profile D-Racer-Kit/src/config/profiles/track.yaml

# quality — 구간(seg.csv)별 BEV 품질. 직선 구간은 폭·수직·평행 오차(validate)
.venv/bin/python offline/bev_eval.py quality <raw.mp4> --camera <cam>.yaml \
    --profile <profile> --segments seg.csv

# coast — coast→pair 전환 진지 오차 + side 뒤집힘률
.venv/bin/python offline/bev_eval.py coast <raw.mp4> --camera <cam>.yaml --profile <profile>
```

## ctrl_feedback — 로그로 제어 파라미터를 되먹이는 도구

제어 튜닝은 실차 폐루프가 최종 판정이지만(아래 open-loop 절 참조), **명령 품질**은 로그로 볼 수
있다. 곡선 e_ss·진동·조향포화·slew포화를 진단하고 PD 관계식으로 권장 kp 를 낸다. 매 실행이 누적
로그에 1행 append 하므로 throttle 다른 주행이 **≥3개** 쌓이면 throttle↔파라미터 추세를 피팅한다.

```bash
.venv/bin/python offline/ctrl_feedback.py <drive.csv> [--segments seg.csv] [--target-cm 17.4]
```

파라미터는 `<drive>.meta.json` 사이드카(recorder 가 활성 `/control/config` 스냅샷으로 기록)에서
자동으로 읽는다. 사이드카가 없으면 `--params kp=1.3 kd=0.145 …` 로 수동 지정한다.
...
> ⚠ **차가 보는 것으로 재라.** 고해상도 지면 사진(960x720)에서는 노랑 H 피크가 17~19 로 나와
> "`yellow_h_lo` 18 이 31% 를 자른다" 는 결론이 나왔고 **그건 틀렸다.** 차는 그 사진을 보지
> 않는다 — 320x240 스트림을 본다. 거기서는 H 가 **18~27** 이고 현재 임계 안에 들어간다.
> (다만 하한에 마진이 없다 — 조명이 어두워지면 `yellow_h_lo` 를 14 로.)

## color_gate_probe — color_gate 가 분기를 숨기는지 세는 도구
...
> ⭐ **지면 사진은 5장을 찍어 서로 대조하라.** 07142150 에서 5장 중 2장이 **혼자서는 완벽해
> 보였다**(카메라 높이 정합, RMS 양호) 지만 차선폭을 35 → 20cm 로 **붕괴시켰다** — 보드가
> 멀어(near 48~52cm) 근거리 `H` 가 무너진 것이다. 나머지가 **서로 독립적으로** 같은 답에
> 모이는 것만이 보증이다. 고르는 기준은 차선폭의 **평균이 아니라 변동**이다.

> ⚠ **`--intrinsics` 를 쓸 때는 `--runtime-size 320x240` 을 반드시 같이 줘라.**
> `--from-camera` 는 런타임 해상도를 물려받지만 `--intrinsics` 는 물려받을 파일이 없다. 안 주면
> **비등방 리스케일**(`fx ×0.333` / `fy ×0.222`)이 저장된다. 로그의 `런타임:` 줄에서 두 배율이
> **같은지** 확인하라.

> ⚠ **보드가 지면에서 떴으면 `--board-offset-cm` 으로 알려줘라.** 안 알려주면 `H` 가 지면이
> 아니라 그 높이의 **유령 평면**으로의 사영이 되고, **숫자는 전부 멀쩡해 보인다** (RMS 는
> 오히려 좋다). 교차검증: 산출된 **카메라 높이가 실측과 맞는지** 보라.
...
`config/profiles/track.yaml` 한 파일.

```yaml
name: track
perception: { <perception_core.Cfg 필드>: <값>, ... }   # 실차 live 튜닝값 유지
control:    { controller: PD, kp: 1.0, kd: 0.13, ... }  # PD | PID
```

**손으로 편집한다.** 자동 writer 는 없다 — PyYAML 이 주석을 보존하지 못하는데, 이 파일의
주석(어떤 측정이 그 값을 정했는지)이 자동 재작성보다 가치가 크다.
